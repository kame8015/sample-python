...
aws ecr describe-repositories --repository-names --profile ${profile_name} "${image}" > /dev/null 2>&1

if [ $? -ne 0 ]
then
    aws ecr create-repository --repository-name --profile ${profile_name} "${image}" > /dev/null
fi
